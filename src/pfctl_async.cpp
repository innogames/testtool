...
PfctlAsync::PfctlAsync(struct event_base *base) : base_(base) {}
...
PfctlAsync::~PfctlAsync() {
  ...
  if (reap_timer_ != nullptr) {
    event_del(reap_timer_);
    event_free(reap_timer_);
    reap_timer_ = nullptr;
  }
}

bool PfctlAsync::run_command(std::vector<std::string> args,
...
void PfctlAsync::spawn_next() {
  ...
  if (pipe(pipefd) < 0) {
    log(MessageType::MSG_CRIT,
        "PfctlAsync: pipe() failed: " + std::string(strerror(errno)));
  ...
  if (pid < 0) {
    log(MessageType::MSG_CRIT,
        "PfctlAsync: fork() failed: " + std::string(strerror(errno)));
    close(pipefd[0]);
    close(pipefd[1]);
    auto cb = std::move(current_callback_);
    current_callback_ = nullptr;
  ...
  pipe_event_ =
      event_new(base_, pipe_fd_, EV_READ | EV_PERSIST, on_pipe_readable, this);
...
void PfctlAsync::try_reap_child() {
  int status;
  pid_t ret = waitpid(child_pid_, &status, WNOHANG);

  if (ret == 0) {
    // Child still running — retry after 10ms.
    struct timeval tv = {0, 10000};
    if (reap_timer_ == nullptr) {
      reap_timer_ = evtimer_new(base_, on_reap_timer, this);
    }
    evtimer_add(reap_timer_, &tv);
    return;
  }

  child_pid_ = -1;

  bool success = (ret > 0 && WIFEXITED(status) && WEXITSTATUS(status) == 0);
...
void PfctlAsync::on_reap_timer(evutil_socket_t fd, short events, void *arg) {
  PfctlAsync *self = static_cast<PfctlAsync *>(arg);
  self->try_reap_child();
}
...
void PfctlAsync::drain() {
  ...
  while (!queue_.empty()) {
    ...
    if (pipe(pipefd) < 0) {
      log(MessageType::MSG_CRIT,
          "PfctlAsync::drain: pipe() failed: " + std::string(strerror(errno)));
      cmd.callback(false, {});
      continue;
    }
    ...
    if (pid < 0) {
      log(MessageType::MSG_CRIT,
          "PfctlAsync::drain: fork() failed: " + std::string(strerror(errno)));
      close(pipefd[0]);
      close(pipefd[1]);
      cmd.callback(false, {});
      continue;
...
bool PfctlAsync::busy() const { return child_pid_ != -1; }
