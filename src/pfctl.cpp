...
bool pf_sync_table(string table, SyncedLbNode *synced_lb_nodes) {
  ...
  // keep information about killing states, so iterate manually instead.
  for (int i = 0; i < MAX_NODES; i++) {
    if (synced_lb_nodes[i].state == LbNodeState::STATE_UP)
      continue;

    bool with_states = !(synced_lb_nodes[i].state == LbNodeState::STATE_DRAIN);
    ...
    for (int proto = 0; proto < 2; proto++) {
      ...
      // Don't run expensive operations on nodes which don't need to be deleted.
