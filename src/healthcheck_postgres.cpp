...
void Healthcheck_postgres::start_conn() {
  ...
  char const *keys[] = {"host",
                        ...
                        NULL};
  char const *values[] = {this->host.c_str(),
                          port_str.c_str(),
                          this->dbname.c_str(),
                          this->user.c_str(),
                          "SQL_ASCII",
...
void Healthcheck_postgres::poll_conn() {
  ...
  case PGRES_POLLING_READING:
    this->register_io_event(EV_READ, &Healthcheck_postgres::poll_conn);
    break;
  ...
  case PGRES_POLLING_FAILED:
    this->end_check(HealthcheckResult::HC_FAIL, "db connection polling failed");
    break;
...
void Healthcheck_postgres::send_query() {
  ...
  if (!PQsendQuery(this->conn, this->query.c_str()))
    return this->register_io_event(EV_WRITE, &Healthcheck_postgres::send_query);
...
void Healthcheck_postgres::handle_query() {
  ...
  if (strlen(val) != 1)
    return this->end_check(HealthcheckResult::HC_FAIL, "db result not 1 char");
...
void Healthcheck_postgres::register_io_event(
    short flag, void (Healthcheck_postgres::*method)()) {
  ...
  if (this->event_counter++ > 100)
    return this->end_check(HealthcheckResult::HC_PANIC, "too many events");
...
void Healthcheck_postgres::register_timeout_event() {
  ...
  this->timeout_event = event_new(
      eventBase, -1, 0, &Healthcheck_postgres::handle_timeout_event, this);
