...
bool pfctl_worker_loop(message_queue* mq) {
	...
	while (running) {
		...
		try {
			...
			mq_success = mq->timed_receive(&msg, sizeof(pfctl_msg), recvd_size, priority, delay);
		}
		catch(const exception& ex)
		{
			log(MSG_CRIT, fmt::sprintf("pfctl_worker: exception while receiving from queue %s", ex.what()));
			return false;
		}
...
message_queue* new_pfctl_queue() {
	...
	catch(const runtime_error& ex)
	{
		...
		log(MSG_INFO, fmt::sprintf("testtool: unable to create queue %s", ex.what()));
	}
	catch(const exception& ex)
	{
	...
	while (true) {
		...
		catch(const interprocess_exception& ex)
		{
			if (ex.get_error_code() == 7) {
				...
				log(MSG_INFO, fmt::sprintf("pfctl_worker: Interprocess Exception while waiting for queue %d", ex.get_error_code()));
			}
		}
		catch(const runtime_error& ex)
		{
			// speciffic handling for runtime_error
			log(MSG_INFO, fmt::sprintf("pfctl_worker: Exception while waiting for queue %s", ex.what()));
		}
		catch(const exception& ex)
		{
			// std::runtime_error which is handled explicitly
