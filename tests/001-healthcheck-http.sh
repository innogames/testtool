...
pool $TABLE1 3
	node $TABLE1_HOST1
		healthcheck http 80 1 2 /:200.403
	node $TABLE1_HOST2
		healthcheck http 80 1 2 /testfile.txt:200.403
	node $TABLE1_HOST3
		healthcheck http 80 1 2 /:200.403
EOF

stage "add 3 nodes to pool" "3 nodes in pool"
ssh $TABLE1_HOST2 'touch /var/www/testfile.txt'
launch_testtool
...
stage "fail a node with wrong http code" "2 nodes in pool"
ssh $TABLE1_HOST2 'rm -f /var/www/testfile.txt'
require_pf_table_waitfor_equal $TABLE1 $TABLE1_HOST1               $TABLE1_HOST3 || exit_fail
stage_end
